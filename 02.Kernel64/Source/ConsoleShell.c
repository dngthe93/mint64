...
static void kMeasureProcessorSpeed(const char *pcParameterBuffer)
{
	...
	// Measure the increment of TSC value for the 10s
	kDisableInterrupt();
	{
		for (i = 0; i < 200; i++)
		{
...
static void kShowDateAndTime(const char *pcParameterBuffer)
{
	...
	tmp = kConvertDayOfWeekToString(bDayOfWeek);
	kPrintf("Date: %d/%d/%d %s, ", wYear, bMonth, bDayOfMonth, tmp);
...
static void kShowTaskList(const char *pcParameterBuffer)
{
	...
	for (i = 0; i < TASK_MAXCOUNT; i++)
	{
		...
		// Only prints currently alive task
		if (pstTCB->stLink.qwID >> 32)
		{
			// Ask user to continue, evertime it prints 10 TCBs
			if (iCount && !(iCount % 10))
			{
				kPrintf("Press any key to coutinue... ('q' is exit): ");
				if (kGetCh() == 'q')
				{
					kPrintf("\n");
					break;
			...
			kPrintf("    Parent PID[0x%Q], MemoryAddress[0x%Q], Size[0x%Q]\n",
...
static void kKillTask(const char *pcParameterBuffer)
{
	...
	if (qwID != 0xFFFFFFFF)
	{
		...
		if ((qwID >> 32) && !(pstTCB->qwFlags & TASK_FLAGS_SYSTEM))
		{
		...
		{
			kPrintf("Task does not exist or task is a system task\n");
		}

	}
	else
	{
		// Kill all tasks except for the console-shell tash and the idle task
		for (i = 0; i < TASK_MAXCOUNT; i++)
		{
			pstTCB = kGetTCBInTCBPool(i);
...
// Vars and Mutex for mutext-testing
...
static void kTestThread(const char *pcParameterBuffer)
{
	...
	if (pstProcess)
		kPrintf("Process [0x%Q] Created Successfully\n", pstProcess->stLink.qwID);
	else
...
static void kShowMatrix(const char *pcParameterBuffer)
{
	...
	if (pstProcess)
	{
		kPrintf("Matrix Process [0x%Q] Created Successfully\n");

		while (pstProcess->stLink.qwID >> 32)
			kSleep(100);
	}
	else
		kPrintf("Matrix Process Creation Failed\n");
}
...
static void kShowDynamicMemoryInformation(const char *pcParameterBuffer)
{
	...
	kPrintf("Start Address: [0xQ]\n", qwStartAddress);
	kPrintf("Total Size: [0xQ]byte, [%d]MB\n", qwTotalSize, qwTotalSize / 1024 / 1024);
	kPrintf("Meta Size: [0xQ]byte, [%d]KB\n", qwMetaSize, qwMetaSize / 1024);
	kPrintf("Used Size: [0xQ]byte, [%d]KB\n", qwUsedSize, qwUsedSize / 1024);
}
